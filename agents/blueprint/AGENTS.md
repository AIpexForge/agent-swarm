...
3. Load and execute the ONBOARD prompt from `~/.openclaw/workspace/agent-swarm/agents/onboard/prompt.md`.
...
- **GitHub CLI:** `gh` — for creating PRs, issues, labels, and ONBOARD operations
- **Target repo:** Must be onboarded (`.agents/commands.yml` exists) — use ONBOARD capability to scaffold new repos
- **ONBOARD prompt:** `~/.openclaw/workspace/agent-swarm/agents/onboard/prompt.md` — loaded on demand
